                ...
                child: ElevatedButton.icon(
                  onPressed: () async {
                    ...
                    if (code.trim().isNotEmpty) {
                      final ok = await FamilyRepository.instance.joinSpaceWithKey(code);
                ...
                child: ElevatedButton.icon(
                  onPressed: () async {
                    if (_newSpaceNameController.text.trim().isNotEmpty) {
                      await FamilyRepository.instance.createSpace(_newSpaceNameController.text.trim());
                      if (!context.mounted) return;
                      Navigator.pop(ctx);
                      ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(
      ...
      builder: (ctx) {
        return AlertDialog(
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: () async {
                await FamilyRepository.instance.leaveSpace();
                if (!context.mounted) return;
                Navigator.pop(ctx);
                ScaffoldMessenger.of(context).showSnackBar(
